...
var/list/department_radio_keys = list(
	  ...
	  ":ê" = "right ear",	"#ê" = "right ear",		".ê" = "right ear", "!ê" = "fake right ear",
	  ":ä" = "left ear",	"#ä" = "left ear",		".ä" = "left ear",  "!ä" = "fake left ear",
	  ...
	  ":ê" = "Response Team","#ê" = "Response Team",".ó" = "Response Team",
	  ":ã" = "Supply",		"#ã" = "Supply",		".ã" = "Supply",
	  ":â" = "Service",     "#è" = "Service",       ".â" = "Service",
	  ":ï" = "changeling",	"#ï" = "changeling",	".ï" = "changeling",
	  ":÷" = "cultchat",	"#÷" = "cultchat",		".÷" = "cultchat",
	  ":í" = "ancientchat",	"#í" = "ancientchat", 	".í" = "ancientchat",
	  ...
	  ":Ô" = "alientalk",	"#Ô" = "alientalk",		".Ô" = "alientalk",
	  ":Å" = "Syndicate",	"#Å" = "Syndicate",		".Å" = "Syndicate",
	  ":Ê" = "Response Team","#Ê" = "Response Team",".Ê" = "Response Team",
	  ":Ã" = "Supply",		"#Ã" = "Supply",		".Ã" = "Supply",
	  ":Â" = "Service",     "#È" = "Service",       ".Â" = "Service",
	  ":Ï" = "changeling",	"#Ï" = "changeling",	".Ï" = "changeling",
	  ":×" = "cultchat",	"#×" = "cultchat",		".×" = "cultchat",
	...
	switch(message_mode)
		...
		if(MODE_CULTCHAT)
			if(construct_chat_check(1)) /*sending check for humins*/
				var/turf/T = get_turf(src)
				log_say("[key_name(src)] (@[T.x],[T.y],[T.z]) Cult channel: [lhtml_encode(speech.message)]")
				var/themessage = text("<span class='sinister'><b>[]:</b> []</span>",src.name,lhtml_encode(speech.message))
				for(var/mob/M in player_list)
		...
		if(MODE_ANCIENT)
			...
			if(stone.len)
				...
				if(commstone.commdevice)
					...
					var/themessage = text("<span class='ancient'>Ancient communication, <b>[]:</b> []</span>",src.name,lhtml_encode(speech.message))
					var/turf/T = get_turf(src)
					log_say("[key_name(src)] (@[T.x],[T.y],[T.z]) Ancient chat: [lhtml_encode(speech.message)]")
