...
/proc/Broadcast_Message(
...
#ifdef SAY_DEBUG
	if(speech.speaker)
		say_testing(speech.speaker, "broadcast_message start - Sending \"[lhtml_encode(speech.message)]\" to [speech.frequency]")
#endif
	...
	if (length(listeners))
		...
		var/enc_message = speech.speaker.say_quote("\"[speech.message]\"") // Does not need to be lhtml_encoded - N3X
