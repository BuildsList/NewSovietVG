...
/proc/command_alert(var/text, var/title = "",var/force_report = 0,var/alert,var/noalert = 0)
	...
	if (title && length(title) > 0)
		command += "<br><h2 class='alert'>[gibberish ? Gibberish(lhtml_encode(title),70) : lhtml_encode(title)]</h2>"


	command += {"<br><span class='alert'>[gibberish_main ? Gibberish(lhtml_encode(text),70) : lhtml_encode(text)]</span><br>
		<br>"}
